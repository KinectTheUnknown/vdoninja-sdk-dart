...
## 2026-08-09 - dart:js_interop Wasm `undefined` regression on `getProperty`
**Learning:** In `dart:js_interop`, redundant `hasProperty` checks can be optimized away by directly accessing the property. However, because JS `undefined` is not implicitly converted to Dart `null` in `dart2wasm` (it is a Wasm `externref`), you must assign the result of `getProperty` to a variable and explicitly check `val.isUndefinedOrNull` before returning Dart `null`. Returning the value directly causes Wasm runtime crashes where `undefined` evaluates as a non-null object.
**Action:** When removing `hasProperty` checks for optimization, always explicitly check the returned value with `.isUndefinedOrNull` before returning or using it as a nullable type in Dart.
