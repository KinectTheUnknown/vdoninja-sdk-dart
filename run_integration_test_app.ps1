...
$statusUrl = "http://localhost:4444/status"
$running = $false
try {
...
if (-not $running) {
    ...
    $CHROMEDRIVER_PROCESS = Start-Process -FilePath $CHROMEDRIVER_BIN -ArgumentList "--port=4444" -PassThru -NoNewWindow
...
Write-Host "🧪 Running SDK integration test app..."
puro flutter drive `
  --verbose `
