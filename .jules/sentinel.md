...
## 2024-05-18 - Data Leakage via Raw Event Logging
**Vulnerability:** Raw WebRTC events and DataChannel messages were being logged to the browser console using `web.console.log()`.
**Learning:** Logging raw events directly to the console can unintentionally leak sensitive data to end-users, browser extensions, or anyone with access to the browser tools. This data can include IP addresses (via ICE candidates), stream keys, passwords, or private chat messages.
**Prevention:** Avoid using `console.log` or similar debug logging for raw WebRTC events or DataChannel messages. If logging is necessary for development, it must be explicitly enabled and should mask or redact sensitive properties before outputting.
