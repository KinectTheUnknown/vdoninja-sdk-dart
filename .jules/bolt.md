...
## 2026-07-26 - Prevent Redundant SDK Script Loading
...
## 2026-07-26 - JS Interop Property Lookup Overhead
**Learning:** Using dynamic JS string conversion and lookup like `event.hasProperty("detail".toJS).toDart` and `event.getProperty("detail".toJS)` inside frequently firing event stream mappings creates immense overhead and garbage collection pressure due to constant string allocations across the Dart/JS boundary.
**Action:** Always use strict typing with `isA<web.CustomEvent>()` and direct getter access `(event as web.CustomEvent).detail` rather than dynamically polling for custom JS properties, especially within hot event callbacks like `onTrack` or WebRTC state streams.
